    ...
    public class MessagesController : Controller
    {
        ...
        public async Task<IActionResult> SendMessage(int applicationId, string message)
        {
            ...
            if (!string.IsNullOrWhiteSpace(message))
            {
                ...
                // Figure out who the recipient is (the other side of this conversation)
                bool senderIsJobSeeker = application.JobSeeker!.ApplicationUserId == senderUserId;
                string recipientUserId = senderIsJobSeeker
                    ? application.Job!.Employer!.ApplicationUserId
                    : application.JobSeeker!.ApplicationUserId;

                string senderLabel = senderIsJobSeeker
                    ? (await _userManager.FindByIdAsync(senderUserId))?.FullName ?? "The applicant"
                    : application.Job!.Employer!.Company?.Name ?? "The employer";
                ...
                _context.Notifications.Add(new Notification
                {
                    ...
                    Message = $"{senderLabel} sent you a message about \"{application.Job!.Title}\".",
                    IsRead = false,
